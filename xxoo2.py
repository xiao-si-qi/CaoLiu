...
def getimg(imgPage,path):
    ...
    img = sup.find_all("img",attrs={"data-src":True,"alt":""})
    for i in img:
        urlstr = parse.quote(i["data-src"])
        imgUrls.append("https://www.2d-erocafe.com"+urlstr)
        print(i["data-src"])
    sevMM(path,imgUrls)
...
def getMenu(url):#获取目录并保存
        ...
        title = sup.find_all("div",class_="entry-card-content")
        print(title)
        for i in  title:
            line=[]
...
def main():
        ...
        for i in menuList:
            tag += 1
            try:
                imgPage = openURL("https://www.2d-erocafe.com"+i[0])
                print(i[1])
                getimg(imgPage, i[1])
            except Exception as e:
                print("出现错误", e)
